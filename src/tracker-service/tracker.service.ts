...
import { TrackerEventBody, TrackerEventType } from './tracker.interface';
...
app.get('/track', (req: Request<null, TrackerEventBody[], TrackerEventBody[]>, res: Response) => {
  const validEvents = req.body.filter(validateEvent);
  res.send(validEvents);
});

function validateEvent(event: TrackerEventBody): boolean {
